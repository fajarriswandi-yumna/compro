...
<div class="row">
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <select class="form-control select2" id="client_id" name="client_id" required {{ isset($bill) ? 'disabled' : '' }}>
            {{-- ADDED: Disable in edit mode --}}
            <option value="" selected disabled>-- Pilih Klien --</option>
            @foreach ($clients as $client)
                <option value="{{ $client->id }}" data-client-id="{{ $client->id }}"
                    data-subscribe-type="{{ $client->subscribe_type }}" data-email="{{ $client->email }}"
                    {{-- ADDED: Select Client if editing and client ID matches --}} {{ isset($bill) && $bill->client_id == $client->id ? 'selected' : '' }}>
                    {{ $client->full_name }} ({{ $client->email }})
                </option>
            @endforeach
        </select>
        @if (isset($bill))
            {{-- ADDED: Hidden input to send client_id in edit mode --}}
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="text" class="form-control" id="no_invoice" name="no_invoice"
            placeholder="Nomor Invoice akan di-generate otomatis" readonly
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="date" class="form-control @error('bill_date') is-invalid @enderror" id="bill_date"
            name="bill_date"
            value="{{ old('bill_date', isset($bill) ? (is_object($bill->bill_date) ? $bill->bill_date->format('Y-m-d') : $bill->bill_date) : '') }}"
            required>
        @error('bill_date')
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date"
            name="due_date"
            value="{{ old('due_date', isset($bill) ? (is_object($bill->due_date) ? $bill->due_date->format('Y-m-d') : $bill->due_date) : '') }}"
            required>
        @error('due_date')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <select class="form-control @error('payment_status') is-invalid @enderror" id="payment_status"
            ...
            <option value="Not Paid"
                {{ old('payment_status', isset($bill) ? $bill->payment_status : '') == 'Not Paid' ? 'selected' : '' }}>
                Belum Dibayar</option> {{-- MODIFIED: Select 'Not Paid' if editing and status is 'Not Paid' --}}
            <option value="Paid"
                {{ old('payment_status', isset($bill) ? $bill->payment_status : '') == 'Paid' ? 'selected' : '' }}>
                Sudah Dibayar</option> {{-- MODIFIED: Select 'Paid' if editing and status is 'Paid' --}}
        </select>
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount"
            value="{{ old('amount', isset($bill) ? $bill->amount : '') }}" required step="0.01" readonly>
        {{-- MODIFIED: Display $bill->amount if editing --}}
        @error('amount')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
<script>
    $(document).ready(function() {
        ...
        @if (isset($bill) && $bill->client_id)
            $('#client_id').val({{ $bill->client_id }}).trigger(
            'change'); // Set Select2 value and trigger change
        @endif
