<div class="row">
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="text" class="form-control @error('full_name') is-invalid @enderror" id="full_name" name="full_name"
            value="{{ old('full_name', isset($client) ? $client->full_name : '') }}" required>
        @error('full_name')
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        <label for="no_phone" class="form-label">No. Telepon <span class="text-danger">*</span></label>
        <input type="text" class="form-control @error('no_phone') is-invalid @enderror" id="no_phone"
            name="no_phone" value="{{ old('no_phone', isset($client) ? $client->no_phone : '') }}" required
            placeholder="Contoh: 628123456789">
        @error('no_phone')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        <small class="text-muted">Using 62 without + and 0</small>
    </div>
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        @if (isset($client) && $client->photo_profile)
            <div class="mt-2">
                <img src="{{ Storage::url($client->photo_profile) }}" alt="Foto Profil" class="img-thumbnail"
                    width="100">
            </div>
        @endif
    </div>
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="text" class="form-control @error('institution') is-invalid @enderror" id="institution"
            name="institution" value="{{ old('institution', isset($client) ? $client->institution : '') }}">
        @error('institution')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <select class="form-select @error('subscribe_status') is-invalid @enderror" id="subscribe_status"
            name="subscribe_status" required>
            <option value="">Pilih Status</option>
            <option value="Aktif"
                {{ old('subscribe_status', isset($client) ? $client->subscribe_status : '') == 'Aktif' ? 'selected' : '' }}>
                Aktif</option>
            <option value="Non Aktif"
                {{ old('subscribe_status', isset($client) ? $client->subscribe_status : '') == 'Non Aktif' ? 'selected' : '' }}>
                Non Aktif</option>
        </select>
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <select class="form-control @error('subscribe_type') is-invalid @enderror" id="subscribe_type"
            name="subscribe_type" required>
            <option value="" selected disabled>-- Pilih Tipe Langganan --</option>
            <option value="Bulanan"
                {{ old('subscribe_type', isset($client) ? $client->subscribe_type : '') == 'Bulanan' ? 'selected' : '' }}>
                Bulanan</option>
            <option value="Tahunan"
                {{ old('subscribe_type', isset($client) ? $client->subscribe_type : '') == 'Tahunan' ? 'selected' : '' }}>
                Tahunan</option>
        </select>
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="date" class="form-control @error('subscription_start_date') is-invalid @enderror"
            ...
            value="{{ old('subscription_start_date', isset($client) && $client->subscription_start_date ? $client->subscription_start_date->format('Y-m-d') : '') }}">
    ...
    <div class="col-md-3 col-sm-6 mb-3">
        ...
        <input type="date" class="form-control @error('subscription_end_date') is-invalid @enderror"
            ...
            value="{{ old('subscription_end_date', isset($client) && $client->subscription_end_date ? $client->subscription_end_date->format('Y-m-d') : '') }}">
        {{-- FORMAT TANGGAL DI SINI --}}
